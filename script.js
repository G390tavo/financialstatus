document.addEventListener("DOMContentLoaded",()=>{
  const btnDark=document.getElementById("btnDark"),
        ...
        navBtns=document.querySelectorAll("aside nav button");
  ...
  iniciarMonedas(); iniciarCriptos(); iniciarEmpresas();
});
...
async function actualizarMoneda(){
  const sel=document.getElementById("selMoneda"),
        selP=document.getElementById("selPeriodoMoneda"),
        grafDiv=document.getElementById("grafMoneda");
...
async function actualizarCripto(){
  const sel=document.getElementById("selCripto"),
        selP=document.getElementById("selPeriodoCripto"),
        grafDiv=document.getElementById("grafCripto");
  grafDiv.innerHTML="Cargando gráfico...";
...
function iniciarEmpresas(){
  ...
  CONFIG.empresasEstaticas.forEach(emp=>{
    ...
    }).catch(()=>{div.innerHTML+= "<br><i>Offline</i>";});
  });
}
...
function render(el,data){
  ...
  data.vals.forEach((v,i)=>{
    ...
    dot.className="punto"; dot.style.left=x+"%"; dot.style.bottom=y+"%"; el.appendChild(dot);
    if(i===data.vals.length-1){
      const lbl=document.createElement("div");
      lbl.className="lbl"; lbl.style.left=x+"%"; lbl.textContent=`$${v.toFixed(2)}`;
      el.appendChild(lbl);
    }
  });
}
